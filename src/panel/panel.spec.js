...
describe('Panel', () => {
  ...
  it('can have html content in header', () => {
    const wrapper = shallow(
      <Panel><header><h2>Example title</h2></header>This is content</Panel>
    );
    expect(wrapper.find('.panel-header')).to.have.length(1);
  ...
  it('can have html content in footer', () => {
    const wrapper = shallow(
      <Panel>This is content<footer><button>OK</button></footer></Panel>
    );
  ...
  it('if collapsible, add accessibility tags', () => {
    const wrapper = shallow(
      <Panel collapsible id="example">
        <header><h2>Example title</h2></header>This is content
      </Panel>
    );
    ...
    expect(wrapper.find('.panel-header'))
      .to.have.attr('aria-controls', 'example-body');
    expect(wrapper.find('.panel-body'))
      .to.have.attr('aria-labelledby', 'example-title');
  });
  ...
  it('could be collapsed', () => {
    ...
    expect(wrapper.find('.panel-header'))
      .to.have.attr('aria-expanded', 'false');
    expect(wrapper.find('.panel-header'))
      .to.have.attr('aria-controls', 'example-body');
    expect(wrapper.find('.panel-body'))
      .to.have.attr('aria-labelledby', 'example-title');
