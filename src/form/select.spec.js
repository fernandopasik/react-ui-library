...
describe('Select', () => {
  ...
  it('with options as an array of strings', () => {
    const
      wrapper = mount(<Select options={ options } />),
      $select = wrapper.find('select');

    expect(wrapper).to.have.exactly(1).descendants('select');
    expect(wrapper).to.have.exactly(3).descendants('option');
    expect($select.childAt(0)).to.have.value('red').and.to.have.text('red');
    expect($select.childAt(1)).to.have.value('green').and.to.have.text('green');
    expect($select.childAt(2)).to.have.value('blue').and.to.have.text('blue');
    ...
    expect(wrapper).to.have.exactly(3).descendants('.option');
    expect($fakeSelect.childAt(0)).to.have.text('red');
    expect($fakeSelect.childAt(1)).to.have.text('green');
    expect($fakeSelect.childAt(2)).to.have.text('blue');
  });
  ...
  it('with options as an array of objects with label and value', () => {
    ...
    expect($select.childAt(0)).to.have.value('')
      .and.to.have.text('Choose');
    expect($select.childAt(1)).to.have.value('red')
      .and.to.have.text('Red Color');
    expect($select.childAt(2)).to.have.value('green')
      .and.to.have.text('Green Color');
    expect($select.childAt(3)).to.have.value('blue')
      .and.to.have.text('Blue Color');
    wrapper.find('.selected-option').simulate('click');

    const $fakeSelect = wrapper.find('.options');
    expect(wrapper).to.have.exactly(1).descendants('.options');
    expect(wrapper).to.have.exactly(4).descendants('.option');
    expect($fakeSelect.childAt(0)).to.have.text('Choose');
